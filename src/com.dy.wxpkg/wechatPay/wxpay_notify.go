...
func NewNaviteNotify(b []byte, wxPayConfig *WxPayConfig) (*NaviteNotify, bool, error) {
	fmt.Println("微信支付返回xml : ", string(b))
	n := &NaviteNotify{}
	//转换
	err := xml.Unmarshal(b, n)
	if err != nil {
		log.D("in NewNaviteNotify xml Unmarshal err is %v", err.Error())
		return n, false, err
	}
	fmt.Printf("xml : %s\n", n)
	if n.Return_code != "SUCCESS" {
		return n, false, errors.New(n.Return_msg)
	}
...
func (n *NaviteNotify) CheckSign(wxPayConfig *WxPayConfig) bool {
	...
	if n.Sign == sign {
		log.D("验证签名successful 获取签名:%v , 计算签名: %v \n", n.Sign, sign)
		return true
	}
	log.D("验证签名错误 获取签名:%v, 计算签名:%v \n", n.Sign, sign)
	return false
}
