...
//set sign and create xml string
func (u *Unifiedorder) SetSign(wxPayConfig *WxPayConfig) string {
	...
	log.D("UnifiedorderRequestXml : ", xml)
	return xml
}

//app pay
//app 支付参数
func (u *Unifiedorder) GenPayReq(wxPay *WxPayConfig, prepay_id string, ono string) ([]byte, error) {
	p := Kvpairs{}
	p = append(p, Kvpair{"appid", wxPay.APPID})
	p = append(p, Kvpair{"noncestr", Md5String(NewOrderNo())})
	p = append(p, Kvpair{"package", "Sign=WXPay"})
	p = append(p, Kvpair{"partnerid", wxPay.MCHID})
	p = append(p, Kvpair{"prepayid", prepay_id})
	p = append(p, Kvpair{"timestamp", GenTimestamp()})
...
func genAppSign(p *Kvpairs, wxPayConfig *WxPayConfig) (string, error) {
...
func (u *Unifiedorder) TakeOrder(wxPayConfig *WxPayConfig) (*UnifiedorderResp, error) {
	...
	log.D("微信预下单返回结果 : ", string(bodyByte))
