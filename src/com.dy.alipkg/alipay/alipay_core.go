...
func verifySign(u url.Values, alipayConfig *AlipayConfig) error {
	...
	if err != nil {
		log.Println("base64EnCode err : ", err)
	}
	log.Println("prestr is : ", prestr)
	log.Println("sign is : ", encodeString, " sign_type is :", alipayConfig.Sign_type)

	switch alipayConfig.Sign_type {
	case "RSA":
		return rsaVerify(prestr, encodeString, alipayConfig.Public_key)
	}
	return fmt.Errorf("not right alipayConfig.Sign_type :", alipayConfig.Sign_type)
}
